...
export default function GenshinInsights() {
  ...
  const characters = genshinAccount.characters;
  const stats = {
    ...
    avgLevel: Math.round(characters.reduce((acc, c) => acc + c.level, 0) / characters.length) || 0,
    totalConstellations: characters.reduce((acc, c) => acc + c.constellation, 0),
  };
  ...
  const elementDistribution = Array.from(elementMap.entries())
  ...
  const weaponDistribution = Array.from(weaponMap.entries())
  ...
  const levelDistribution = levelRanges.map((range) => ({
    range: range.range,
    count: characters.filter((c) => c.level >= range.min && c.level <= range.max).length,
  }));
